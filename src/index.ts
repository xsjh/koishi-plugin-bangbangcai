...
export const Config =
  Schema.intersect([
    Schema.object({
      bbc: Schema.string().default("猜猜看").description("`父级指令`名称"),
      ...
      bbc_bzd_command: Schema.string().default("bzd").description("`不知道答案`的指令名称"),
    }).description('基础设置'),
    Schema.object({
...
export async function apply(ctx: Context, config) {
  ...
  // 存储每个群聊的监听器
  const listeners: Record<string, (session: any) => void> = {};
  // 存储每个群聊的计时器
  const timers: any = {};
  ...
  ctx.on('ready', async () => {
    ...
    try {
      ctx.model.extend("bangguess_user",
        ...
        },
        {
          // 使用自增的主键值
          autoInc: true,
          unique: [["platform", "channelId"]], // userId 从 unique 移除，改为 channelId 维度
        }
      );
    } catch (error) {
    ...
    ctx.i18n.define("zh-CN",
      {
        commands: {
          ...
          [config.bbc_bzd_command]: { // bzd 指令 i18n
            description: "不知道答案",
            messages: {
              "nogame": "当前没有正在进行的游戏哦~",
            }
    ...
    ctx.command(`${config.bbc}/${config.bbc_restart_command}`)
      .action(async ({ session }) => {
        ...
        try {
          await ctx.database.remove("bangguess_user", { channelId }); // 清理整个 channelId 的数据，包括 gaming 为 true 的
    ...
    ctx.command(`${config.bbc}/${config.bbc_bzd_command}`)
      .action(async ({ session }) => {
        ...
        try {
          ...
          await clearGameSession(channelId, userId); // 结束游戏 session, 传入 userId
        } catch (error) {
    ...
    // 游戏结束后清除对应用户群聊数据库方法 (修改为只更新 gaming 状态)
    async function clearGameSession(channelId: string, userId?: string) { // 增加 userId 参数
      try {
        if (channelId) {
          const gameRecord = await ctx.database.get("bangguess_user", { channelId, gaming: true });
          if (config.autocleantemp && gameRecord.length > 0) { // 检查配置项和游戏记录是否存在
            ...
            const userFolder = path.join(card_path, "images", `${currentUserId}_${channelId || ""}`);
            ...
            await ctx.database.remove("bangguess_user", { channelId, gaming: true }); // 删除 gaming 为 true 的记录
            logInfo(`已删除群聊 ${channelId} 的游戏数据 (autocleantemp enabled)`);

          } else {
            await ctx.database.set("bangguess_user", { channelId, gaming: true }, { gaming: false }); // 更新 gaming 状态为 false
          ...
          if (listeners[channelId]) {
            // listeners[channelId](session); // 调用监听器移除函数
        ...
        try {
          ...
          try {
            const existingGame = await ctx.database.get("bangguess_user", {
              channelId, gaming: true // 查找 gaming 为 true 的记录
            });
            if (existingGame.length > 0) {
              ...
              const data = {
                ...
                channelId: session.channelId || "sandbox", // 根据需要处理 channelId 是否为空
                time: new Date(),
          ...
          try {
            await ctx.database.set("bangguess_user",
              { channelId: channelId || "sandbox", gaming: true }, // 查找 gaming 为 true 的记录
          ...
          const messageListener = async (session) => {
            ...
            if (nicknames.some((nickname) => userInput === (nickname))) {
              ...
              try {
                ...
                logInfo('回答正确消息发送至：', userId, userInput)
              } catch (error) {
    ...
    async function randomCropImage(
      ...
      try {
        ...
        // 创建文件夹路径
        const userFolder = path.join(folderPath, `${userId}_${channelId || ""}`);
        if (!fs.existsSync(userFolder)) {
