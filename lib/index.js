...
(如果猜不出来输入 bzd 可以查看答案)`;
    ...
    const dispose = ctx.channel(session.channelId).middleware(async (session2, next) => {
      if (nicknames.includes(session2.content)) {
        dispose();
        disposeTimer();
        games[session2.channelId] = false;
        await session2.send(`正确，${nicknames[7]}：${imageUrl}`);
        await session2.send(`${import_koishi.h.at(session2.userId)}${config.phrase_answered}${session2.content}
${import_koishi.h.image(image, "image/jpeg")}`);
      } else if (session2.content === "bzd") {
        dispose();
        disposeTimer();
        delete games[session2.channelId];
