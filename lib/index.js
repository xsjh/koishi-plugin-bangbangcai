      ...
      try {
        ...
        try {
          const existingGame = await ctx.database.get("bangguess_user", {
            ...
            gaming: true
            // 查找 gaming 为 true 的记录; // 使用 sanitizedChannelId 查询;
          });
        ...
        try {
          await ctx.database.set(
            ...
            // 查找 gaming 为 true 的记录; // 使用 sanitizedChannelId 更新;
        ...
        const messageListener = /* @__PURE__ */ __name(async (session2) => {
          ...
          if (!timers[channelId] && !config.nowtimers) {
            timers[channelId] = ctx.setTimeout(async () => {
              try {
                ...
                logInfo("[倒计时] 超时游戏结束消息发给了", channelId, userId2);
              } catch (error) {
                ctx.logger.error("[倒计时] 发送消息或图片时出错:", error);
              } finally {
                logInfo("[倒计时] 超时, 弃置监听器");
                await clearGameSession(channelId, userId2, session2.isDirect);
              }
          ...
          if (nicknames.some((nickname) => userInput === nickname)) {
            ...
            try {
              ...
              logInfo("[回答正确] 回答正确消息发送至：", userId3, userInput);
            } catch (error) {
              ctx.logger.error("[回答正确] 发送消息时出错:", error);
            } finally {
              await clearGameSession(channelId, userId3, session2.isDirect);
            }
          }
        }, "messageListener");
